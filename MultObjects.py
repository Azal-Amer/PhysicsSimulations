...
for n in range(count):
    coordinates = randomCords(radius,inner_Radius)     
    x = coordinates[0]
    y = coordinates[1]
    z = coordinates[2]
...
#     vp.s[n] = vp.sphere(radius=.1,pos=vp.vector(0,((n)/10)+4,0),mass = 1, momentum = vp.vector(0,0,0), make_trail = False, motion = True)
#     #     print(str(n)  + "- " + "distance : " + str(vp.mag(vp.s[n].pos)) + ", radius: " + str(radius) )
...
while(run):
    ...
    for n in range(count):
        ...
        if vp.s[n].motion == True:
            ...
            if abs(vp.mag(vp.s[n].momentum)) < .01:
                # when the momentum is hitting a turning point
                mmnt_curve.plot(n, abs(vp.mag(vp.s[n].pos)))
